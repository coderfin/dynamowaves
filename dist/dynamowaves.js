const vertical_paths = Object.freeze([
...
const horizontal_paths = Object.freeze([
...
function observeWave(wave, v) {
	...
	const observer = new IntersectionObserver(
		([e]) => {
			if (!e.isVisible) {
				e.target.nextElementSibling.children[0].setAttribute(
					"d",
					v
						? shuffleArray(vertical_paths)[0]
						: shuffleArray(horizontal_paths)[0]
				);
			}
		},
		{
...
class DynamoWave extends HTMLElement {
	...
	connectedCallback() {
		...
		let vertical =
			wave_direction == "left" || wave_direction == "right"
				? true
				: false;

		if (this.getAttributeNode("data-wave-observe")) {
			observeWave(this, vertical);
    }

		let path = vertical
			? shuffleArray(vertical_paths)[0]
			: shuffleArray(horizontal_paths)[0];
		let wave_seed = this.getAttribute("data-wave-seed");
		if (wave_seed && vertical && vertical_paths[wave_seed]) {
			path = vertical_paths[wave_seed];
		} else if (wave_seed && !vertical && horizontal_paths[wave_seed]) {
			path = horizontal_paths[wave_seed];
		}
    
		this.outerHTML = `
          <svg viewBox="${
				vertical ? "0 0 100 500" : "0 160 1440 160"
			}" preserveAspectRatio="none" class="${
			classes ? classes : ""
		}" style="${flip_x ? "transform:scaleX(-1);" : ""}${
			flip_y ? "transform:scaleY(-1);" : ""
		}${styles ? styles : ""}" ${id ? `id="${id}"` : ""}>
            <path d="${path}" style="stroke: none; fill: inherit" ${
			vertical
				? 'transform="matrix(-0.000001, -1, 1, -0.000001, -192.292949, 292.569243)"'
				: ""
		}></path>
          </svg>
        `;
